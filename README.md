...
## 💳 随缘付（ChancePay）｜分布式支付一致性系统 MVP  
**个人原创项目｜Spring Boot · MySQL · RabbitMQ**

随缘付是我独立设计与实现的支付编排系统，用于验证 **分布式事务一致性** 与 **幂等性控制**。  
系统核心理念：**任何失败都必须有记录，所有状态都能被解释。**  
无论在 MQ、网络或节点任意处挂掉，记录都不会丢失或陷入未知。
...
3. **Inbox**：消费者通过 `INSERT IGNORE` 去重插入，确保幂等处理。  
4. **Callback**：Wallet 回调 Orchestrator 更新最终状态（`TRUE`/`FALSE`）。

每一步都可观测、可复盘——系统永远知道自己“停在哪一步”。

---

### ✳️ 核心特性
- **事务外盒 Outbox**：写库与发消息解耦但强一致，防丢单。  
- **消费端幂等**：`INSERT IGNORE` + 唯一键，天然抵抗 MQ 重投。  
- **三态状态机**：`NULL / TRUE / FALSE` 表达流程单向且完备。  
- **可追溯失败**：任何异常都会在表或日志中留下“确切痕迹”。  

---

### 🧠 系统哲学
> 一致性不是追求完美同步，而是拒绝迷失。  
> 系统可以延迟，但不能模糊；可以失败，但必须知道为什么。

---

**结果：**  
构建出一个可靠、可部署、可解释的支付系统 MVP。  
展示了我在 **事务边界、幂等控制、最终一致性与系统可观测性** 方面的工程能力。
